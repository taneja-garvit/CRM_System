  ...
  try {
    ...
    const reqBody = {
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a precise marketing assistant and must follow the user instructions exactly.'
        },
        {
          role: 'user',
          content: fullPrompt,
    ...
    // LOG the raw response for debugging (server-side)
    ...
    if (typeof content === 'object') {
      // if it's an object, stringify a reasonable part
    ...
    if (!content) {
      // include a truncated debug snapshot in the thrown error so catch() logs it
      const snapshot = JSON.stringify(response.data).slice(0, 2000);
      throw new Error(`No message returned in expected fields. Response snapshot: ${snapshot}`);
    }

    // Limit to first sentence or 100 chars (your original behavior)
    let generatedMessage = content.split('.')[0].substring(0, 100);
    ...
    // Return limited debug info to client (you can remove `details` in production)
    const details = (error.response && error.response.data) ? error.response.data : (error.message || 'Unknown error');
    res.status(500).json({ error: 'Failed to generate message with AI', details });
  }
};
